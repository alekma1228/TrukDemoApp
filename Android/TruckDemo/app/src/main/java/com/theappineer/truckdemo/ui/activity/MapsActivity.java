...
import static com.theappineer.truckdemo.AppConstant.bingmap_base_url;
import static com.theappineer.truckdemo.AppConstant.bingmap_sub_url_async_callback;

public class MapsActivity extends AppCompatActivity implements OnMapReadyCallback, MapboxMap.OnMapLongClickListener, PermissionsListener {
...
//        mMapboxMap.animateCamera(CameraUpdateFactory.newCameraPosition(position), 1000);
        ...
        LatLngBounds latLngBounds = new LatLngBounds.Builder()
                .include(new LatLng(curLat, curLng))
                .include(new LatLng(destLat, destLng))
                .build();
        mMapboxMap.animateCamera(CameraUpdateFactory.newLatLngBounds(latLngBounds, 10, 100, 10, 10));
    ...
    private void getStatusRequestFromBingMap(final String requestId){
        this.requestId = requestId;
        String url = bingmap_base_url + bingmap_sub_url_async_callback + "?";
        url = url + "requestId=" +requestId;
        url = url + "&key=" +AppConstant.bingmap_key;
        new HttpRequestTask.HttpAsyncTask(dlg_progress, mBingMapStatusResponseListener).execute(url, "");
    }

    private void getStatusRequestToBingMap(final String requestId){
//        mRetrofit = new Retrofit.Builder()
//                .baseUrl(bingmap_base_url)
...
//                    try {
//                        MessageUtil.showError(MapsActivity.this, response.errorBody().string());
//                    } catch (IOException e) {
//                        MessageUtil.showError(MapsActivity.this, "Error Body parsing issue.");
//                        e.printStackTrace();
//                    }
//                    return;
//                }
...
//                    getStatusRequestToBingMap(requestId);
                    getStatusRequestFromBingMap(requestId);
                } else {
                    MessageUtil.showError(MapsActivity.this, "mBingMapResponse is empty");
                    dlg_progress.dismiss();
                }
            } catch (Exception ex) {
                MessageUtil.showError(MapsActivity.this, "mBingMapResponse parsing error");
                dlg_progress.dismiss();
            }
        }
    };
    private HttpRequestTask.HttpResponseListener mBingMapStatusResponseListener = new HttpRequestTask.HttpResponseListener() {
        @Override
        public void onResponse(String ret) {
            try {
                if (!ret.isEmpty()) {
                    ...
                    String requestId = item2.getString("requestId");
                    String resultUrl = item2.getString("resultUrl");
                    String errorMessage = item2.getString("errorMessage");

                    if (errorMessage.equals("Request completed")){
                        downloadResultFromBingMap(resultUrl);
                    } else { //if (errorMessage.equals("No solution.")){
                        MessageUtil.showError(MapsActivity.this, errorMessage);
                        dlg_progress.dismiss();
                    }
                } else {
                    new Handler().postDelayed(new Runnable() {
        ...
        public void onResponse(String response) {
            try {
                if (!response.isEmpty()) {
                    ...
                    MessageUtil.showError(MapsActivity.this, "Download request's response is empty");
                    dlg_progress.dismiss();
                }
            } catch (Exception ex) {
                MessageUtil.showError(MapsActivity.this, "Download request's response parsing error");
                dlg_progress.dismiss();
            }
        }
    };
